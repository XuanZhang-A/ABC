...
class BaseTrainer:
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, config, **kwargs):
        ...
        self.logging = config.config.get("logging", None)         
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.mnt_mode != 'off':
                ...
                self.logger.info('    {:15s}: {:0.4f}'.format('current best', self.mnt_best))
    ...
    def _save_checkpoint(self, epoch, save_best=False, best_only=False):
        ...
        if not best_only:
            ...
            self.write_metrics(self.train_metrics.result(), self.valid_metrics.result(), epoch)
        if save_best:
    ...
    def write_metrics(self, train_metrics, val_metrics, epoch):
        self.metrics = {}
        for key, value in train_metrics.items():
            self.metrics['train_' + key] = float(value)
        
        for key, value in val_metrics.items():
            self.metrics['val_' + key] = float(value)
        
        best_val_acc = self.metrics.get('best_val_accuracy', None)
        if best_val_acc is None or best_val_acc < val_metrics['accuracy']:
            self.metrics['best_val_accuracy'] = float(val_metrics['accuracy'])
            self.metrics['best_val_epoch'] = epoch
        write_experiment_finding(self.metrics, str(self.config.save_dir), "metrics")
