...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_experts, num_classes=10, 
                 ...
                 layer3_output_dim=None, use_norm=False, returns_feat=True, 
                 use_experts=None, s=30):
        ...
        self.layer2s = self._make_layer(block_, layer2_output_dim, num_blocks[1], 
                                        stride=2, groups=self.groups)

        self.in_planes = self.next_in_planes
        self.layer3s = self._make_layer(block_, layer3_output_dim, num_blocks[2], 
                                        stride=2,groups=self.groups)
        self.in_planes = self.next_in_planes
    ...
    def _hook_before_iter(self):
        assert self.training, "_hook_before_iter should be called at training \
                               time only, after train() is called"
        count = 0
        ...
        if count > 0:
            print("Warning: detected at least one frozen BN, \
                  set them to eval state. Count:", count)
    ...
    def forward(self, x):
        ...
        # projection head part
        self.expert_feats = torch.concat(self.expert_feats, dim=1)
        z = self.projection_head(self.expert_feats)
        p = self.contrast_head(z)
        self.feats.append(self.expert_feats)
        # self.feats = torch.stack(self.feats, dim=1) # 特征, 在linear之前
